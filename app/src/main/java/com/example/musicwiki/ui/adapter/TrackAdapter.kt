...
class TrackAdapter : RecyclerView.Adapter<BaseViewHolder<*, *>>() {
    ...
    class MyAlbumViewHolder(private val binding: ItemAlbumBinding) :
        BaseViewHolder<Track, OnTrackClickListener>(binding) {
        override fun bind(item: Track, listener: OnTrackClickListener?) {
            val data = DataModel(item.name, item.image!![0].text)
            binding.data = data
            binding.layout.setOnClickListener {
                listener?.trackClickListener(item)
            }
        }
    }

    private val albums = ArrayList<Track>()
    private var listener: OnTrackClickListener? = null

    fun setData(albums: ArrayList<Track>, listener: OnTrackClickListener? = null) {
    ...
    private fun setTag(string: String) {
        val tag = Track(null, "0", null, "", string, "0")
